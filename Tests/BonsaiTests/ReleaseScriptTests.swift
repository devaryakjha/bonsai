...
final class ReleaseScriptTests: XCTestCase {
  ...
  func testReleaseWorkflowVerifiesArtifactsBeforeUploadAndCleansTemporaryKeychain() throws {
    ...
    let dryRunBuildRange = workflow.range(of: "name: Build dry-run archive")
    let notarizedBuildRange = workflow.range(of: "name: Build signed and notarized archive")
    ...
    XCTAssertTrue(workflow.contains("BONSAI_RELEASE_DRY_RUN=$dry_run"), workflow)
    ...
    XCTAssertTrue(workflow.contains("if: ${{ inputs.dry_run == true }}"), workflow)
    XCTAssertTrue(workflow.contains("if: ${{ inputs.dry_run != true }}"), workflow)
    XCTAssertTrue(workflow.contains("./script/package_release.sh --verify-archive"), workflow)
    XCTAssertTrue(workflow.contains("./script/package_release.sh --notarize"), workflow)
    ...
    XCTAssertNotNil(dryRunBuildRange)
    XCTAssertNotNil(notarizedBuildRange)
    XCTAssertLessThan(dryRunBuildRange?.lowerBound ?? workflow.endIndex, verifyRange?.lowerBound ?? workflow.startIndex)
    XCTAssertLessThan(notarizedBuildRange?.lowerBound ?? workflow.endIndex, verifyRange?.lowerBound ?? workflow.startIndex)
