...
final class ReleaseScriptTests: XCTestCase {
  ...
  func testReleaseScriptDocumentsAndChecksArtifactVerifier() throws {
    ...
    XCTAssertTrue(script.contains("sign_archive()"))
    XCTAssertTrue(script.contains("codesign --verify --strict --verbose=2 \"$ARCHIVE_PATH\""))
    XCTAssertTrue(script.contains("create_archive \"$(developer_id_identity)\""))
